"""
Improved AWS CDK Stack for Private API Gateway with Fan-out Pattern
"""
...
class APIGWStack(Stack):
    ...
    def __init__(self, scope: Construct, id: str, props: Dict[str, Any], **kwargs) -> None:
        ...
        created_status_queue, other_status_queue = self._create_sqs_queues_and_subscriptions(topic)
    ...
    def _create_nlb_and_vpc_link(self, vpc: ec2.Vpc) -> tuple[elbv2.NetworkLoadBalancer, apigw.VpcLink]:
        ...
        health_listener.add_targets(
            "HealthTarget",
            port=80,
            targets=[elbv2.NetworkLoadBalancerTarget.from_ip_address(vpc.private_subnets[0].ipv4_cidr_block.split('/')[0])]
        )
